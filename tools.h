...
namespace XKERB
{
	...
	std::wstring XKList()
	{
		...
		for (ULONG i = 0; i < Cache->CountOfTickets; i++)
		{
			...
			stream << L'\t' << L"Ticket Flags 0x" << std::setfill(L'0') << std::hex << Ticket.TicketFlags << std::dec << std::setfill(L' ') << L' ' << TicketFlagsStream.str() << std::endl;
