...
void test_submit_client_tkt()
{
...

	{
		...
		res = XKERB::XCallAuthenticationPackage<KerbRetrieveEncodedTicketMessage>({
			...
			.CacheOptions = KERB_RETRIEVE_TICKET_CACHE_TICKET /*KERB_RETRIEVE_TICKET_DONT_USE_CACHE */ | KERB_RETRIEVE_TICKET_AS_KERB_CRED
			});

		std::cout << "============= Client Tokens ======================" << std::endl;

		std::wstring wres = XKERB::XKList();
		std::wprintf(L"%ws\n", wres.c_str());
...
void test_submit_tkt_2()
{
	...
	auto res = XKERB::XCallAuthenticationPackage<KerbRetrieveEncodedTicketMessage>({
		.TargetName = std::format(L"ldap/{}@{}", TEST_SERVER, TEST_DOMAIN),
		.CacheOptions = KERB_RETRIEVE_TICKET_CACHE_TICKET | KERB_RETRIEVE_TICKET_AS_KERB_CRED,
...
int main()
{
	...
	// test_pin_unpin_kdc(); // работает только на контроллере домена!
	test_submit_client_tkt();
